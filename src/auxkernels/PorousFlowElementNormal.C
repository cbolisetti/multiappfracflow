...
registerMooseObject("csiroApp", PorousFlowElementNormal);

template <>
InputParameters
validParams<PorousFlowElementNormal>()
{
  InputParameters params = AuxKernel::validParams();
  MooseEnum component("x=0 y=1 z=2");
  params.addRequiredParam<MooseEnum>("component", component, "The component to compute");

  params.addParam<RealVectorValue>("3D_default", RealVectorValue(0, 0, 1), "The value that will be produced for 3D elements, since such elements do not have a 'normal direction'");
  params.addParam<RealVectorValue>("1D_perp", RealVectorValue(0, 0, 1), "The normal for all 1D elements will be perpendicular to this vector");

  params.addClassDescription("AuxKernel to compute components of the element normal.  This is mostly designed for 2D elements living in 3D space, however, the 1D-element and 3D-element cases are handled as special cases.  The Variable for this AuxKernel must be an elemental Variable");
...
PorousFlowElementNormal::computeValue()
{
  ...
  switch(_current_elem->dim())
      ...
      for (unsigned i = 0; i < num_nodes - 2; ++i)
	{
	  ...
	  RealVectorValue v2 = _current_elem->point((i + 2) % num_nodes) - _current_elem->point((i + 1) % num_nodes);
	  n += v1.cross(v2);
	}
      break;
    }
